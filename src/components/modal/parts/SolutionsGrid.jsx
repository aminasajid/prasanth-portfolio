  ...
  return (
    <div className='mt-16'>
      <div className='mb-5'>
        ...
        <h3 className="text-4xl md:text-5xl font-bold leading-[48px]" style={{ color: accent }}>What we built</h3>
      </div>

      {/* Small screens: horizontal scroll with controls; md+: grid 1/2/3 */}
      <div className='relative'>
        ...
        {solutions.map((s, i) => (
          <div
            key={i}
            className="self-stretch h-[500px] rounded-4xl relative bg-gradient-to-b from-[#b6b6b6]/0 snap-start shrink-0 md:shrink min-w-[85%] xs:min-w-[75%] sm:min-w-[60%] md:min-w-0"
            style={{
              backgroundImage: `linear-gradient(to bottom, rgba(182,182,182,0) 0%, ${accent}66 100%)`,
            }}
            // Ensure wide cards in horizontal flow
            
          >
            <div className="h-full relative rounded-[32px] overflow-hidden">
              <div className="absolute left-1/2 -translate-x-1/2 bottom-[80px] w-[85%] h-[140px] "></div>

              {s.image && (
                <img
                  src={s.image}
                  alt={s.title || 'Solution image'}
                  className={s.imageClassName || 'absolute left-1/2 -translate-x-1/2 bottom-0 w-[90%] h-[240px] object-contain rounded-lg'}
                />
              )}

              <div className="absolute top-8 left-6 right-6 px-3 inline-flex flex-col gap-2">
                {s.title && (
                  <div className="text-black font-onset text-[32px] font-bold leading-[130%] tracking-[-0.554px]">
                    {s.title}
                  </div>
                )}
                {s.description && (
                  <div className="text-black font-onset text-lg leading-[130%] tracking-[-0.554px]">
                    {s.description}
                  </div>
                )}
              </div>
