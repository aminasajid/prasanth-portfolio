...
// Blue theme - Grid layout with 6 screens (mobile/tablet mix)
    ...
    <div className="mt-16 md:mt-20">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {images.slice(0, 6).map((img, idx) => (
          <div 
            key={idx}
            className="relative rounded-3xl md:rounded-[32px] p-6 md:p-8 flex items-center justify-center overflow-hidden"
            style={{ 
              background: `linear-gradient(135deg, ${bgColor} 0%, ${bgColor}dd 100%)`,
              minHeight: idx >= 3 ? '400px' : '450px'
            }}
          >
            <img
              src={img}
              alt={`Screen ${idx + 1}`}
              className="w-full h-auto max-w-[280px] rounded-2xl md:rounded-3xl shadow-2xl object-contain"
            />
          </div>
        ))}
      </div>

      {/* Bottom large section for additional screens */}
      {images.length > 6 && (
        <div 
          className="w-full mt-4 md:mt-6 rounded-3xl md:rounded-[32px] p-6 md:p-12 flex items-center justify-center gap-6 overflow-hidden"
          style={{ backgroundColor: bgColor }}
        >
          {images.slice(6, 8).map((img, idx) => (
            <div key={idx} className="relative flex-1 max-w-[500px]">
              <img
                src={img}
                alt={`Additional screen ${idx + 1}`}
